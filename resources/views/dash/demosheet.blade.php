    ...
    if (count($usecredentials) > 0) {
        ...
        <form action="{{ route('crudemarks.store') }}" method="post">
           ...
           <!-- Add the hidden input field for topic ID -->
            ...
            <select name="setTopic" id="setTopic" onchange="storeTopicId()">
...
function updateTopicId()
 {
        ...
        topicSelect.innerHTML = ""; // Clear previous options after selecting another subject
        ///var topicIdInput = document.getElementById("topicid");  Not so important

        // Find the matching topics based on the selected subject
        var matchingTopics = topics.filter(function(topic) {
            return topic.subjectid === selectedSubject;
        });

        // Create new options for the topic select element
        ...
        // Find the matching topic ID based on the selected subject
       var selectedTopic = topics.find(function(topic) {
           return topic.subjectid === selectedSubject;
        });
            // Update the value of the hidden input field with the selected topic ID
        topicIdInput.value = selectedTopic ? selectedTopic.topicid : '';
        ///topicIdInput.value = selectedTopic.topicid;
        //document.getElementById("topicid").value = selectedTopic.topicid;
